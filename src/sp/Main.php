...
{
    public $prefix = TE::GRAY . "[" . TE::AQUA . TE::BOLD . "" . TE::RED . "Dragons" . TE::RESET . TE::GRAY . "]";
    public $game = [];
    public $levels = array();
	...
	public function onEnable()
    {
        if ($this->getServer()->getPluginManager()->getPlugin("EconomyAPI") !== null) {
            $this->getLogger()->info("Plugin Dragons enabled!");
            $this->getLogger()->info("Dragons github: github.com/callumrawlinson");
    ...
    {
        if ($cmd->getName() == "dragons") {
            if (!$s instanceof Player) {
                $s->sendMessage("Run this command on game");
                return false;
            }
            if (count($args) < 2) {
                $s->sendMessage("/dragons make Arena Name");
                return false;
            }
            $level = $args[1];
